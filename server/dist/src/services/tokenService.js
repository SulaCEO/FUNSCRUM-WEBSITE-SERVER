...
const createJwtTokens = (payload) => {
    if (process.env.ACCESS_KEY && process.env.REFRESH_KEY) {
        const accessToken = jsonwebtoken_1.default.sign(payload, process.env.ACCESS_KEY, { expiresIn: '15m' });
        const refreshToken = jsonwebtoken_1.default.sign(payload, process.env.REFRESH_KEY, { expiresIn: '30d' });
        return {
            accessToken,
            refreshToken
        };
